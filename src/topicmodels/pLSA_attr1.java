...
public class pLSA_attr1 extends twoTopic {
	...
	public pLSA_attr1(int number_of_iteration, double converge, double beta, _Corpus c, //arguments for general topic model
	...
	protected void initialize_probability(Collection<_Doc> collection) {	
		...
		for(int i=0;i<number_of_topics/2;i++)
			Utils.randomize(this.topic_term_probabilty[i], d_beta-1.0);
		
		int k = number_of_topics/2;
		for(int i=k; i<number_of_topics; i++)
			System.arraycopy(topic_term_probabilty[i-k], 0, topic_term_probabilty[i], 0, vocabulary_size);
	}
	...
	public double calculate_E_step(_Doc d) {	
		...
		if(d.test == 1)
		{
			...
			double word_topic[][];
			//System.out.println("Senlen"+sentences.length);
			d.m_sentence_topic = new int[sentences.length];
			//System.out.println("Complete");
			
			//System.out.println();
			for(int s=0; s<sentences.length; s++)
			{
				word_topic = new double[sentences[s].length][this.number_of_topics]; // word by topic number of wor in snetence
				//System.out.println("Complete"+sentences[s].length);
				for(int w=0; w<sentences[s].length; w++) // scan through all the words w in the sentence i
				{
					int j = sentences[s][w].getIndex();
					double v = sentences[s][w].getValue();
				...
				// compute the argmax_z p(z|s,d)
				double max = 0.0;
				for(int k=0;k<this.number_of_topics;k++) {
					double result = 1.0;
					for(int w=0; w<sentences[s].length; w++){
						result = result * word_topic[w][k];
					}
					if(result>max)
					{
						max = result;
						d.m_sentence_topic[s] = k;
					}
		...
		{//NewEgg
			...
			// for Cons section k = second half
			for(_SparseFeature fv:d.getSparseneg()) {
				int j = fv.getIndex(); // jth word in doc
	...
	public void document_summary(int topic)
	{
		...
		int index = -1;
		//int neg_topic = topic + this.number_of_topics/2;
		
		int neg_topic = mapping[topic];
					...
					if(d.getAllSentencesneg()!=null)
					{
						...
						for(int s=0; s<neg_sentence.length; s++)
						{
							...
							for(int w=0; w<neg_sentence[s].length;w++)
							{
								...
								prod = prod * topic_term_probabilty[neg_topic][j]*d.m_topics[neg_topic];
							}
