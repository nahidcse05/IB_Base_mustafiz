...
public class pLSA_attr extends twoTopic {
	...
	public pLSA_attr(int number_of_iteration, double converge, double beta, _Corpus c, //arguments for general topic model
	...
	public double calculate_E_step(_Doc d) {	
		...
		for(_SparseFeature fv:d.getSparseneg()) {
			...
			//System.out.println("Neg Feature:" + v);
...
/*		if(d.from == 1) // that means doc from new Egg so do regularization
		{
			this.new_egg_doc_counter++;
			double gdelta = 1e-5, istp = 1.0;
			int maxStep = 50;
			problem_prattrb testcase = new problem_prattrb(d.m_topics,d.m_topic_attribute_sstat);			
	...
	public void calculate_M_step() {
		...
		if(this.new_egg_doc_counter!=0)
			System.out.println("Success Rate:" + (double)(optimizer_success_count/new_egg_doc_counter) +"\n");
	...
	public void printTopWords(int k) {
		...
		for(int i=0; i<topic_term_probabilty.length; i++) {
			...
			for(_RankItem it:fVector)
				System.out.format("%s(%.15f)\t", it.m_name, it.m_value);
			System.out.println();
		}
	}
