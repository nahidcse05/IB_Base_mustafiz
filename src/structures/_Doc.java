...
public class _Doc implements Comparable<_Doc> {
	...
	public int from; // from different source from = 1 means newegg doc and from = 2 amazon data
	//public int section; // section = 1 means pros and section = 2 means cons section = 3 means both mixed
	...
	public int m_predict_label; //The predicted result.
	...
	public _SparseFeature[] m_x_sparse; // sparse representation of features: default value will be zero.
	public _SparseFeature[][] m_sentences; // sentence array each row contains the unique word id 
	public _SparseFeature[][] m_sentences_neg; // sentence array each row contains the unique word id 
	
	public _SparseFeature[] m_x_sparse_neg;
	
	
	public int test = 0; // 0 means it is training doc; 1 means it is for testing
	...
	public double[][] m_topic_attribute_sstat; // p(a | z, d)
	...
	public void update_source(String source)
	{
		this.m_source = source;
	}
	...
	//Get the sparse vector of the document.
	public _SparseFeature[] getSparseneg(){
		return this.m_x_sparse_neg;
	}
	...
	public void createSpVct1(HashMap<Integer, Double> spVct) {
		m_x_sparse = Utils.createSpVct1(spVct);
	...
	//Create the sparse vector for the document.
	public void createSpVct(HashMap<Integer, wordLabel> spVct) {
	...
	//Create the sparse vector for the document.
	public void createSpVctneg(HashMap<Integer, wordLabel> spVct) {
		m_x_sparse_neg = Utils.createSpVct(spVct);
		for(_SparseFeature fv:m_x_sparse_neg)
			m_totalLength += fv.getValue();
	}
		
	
	//Create the sparse vector for the document.
	public void createSpVctAmazon(HashMap<Integer, Double> spVct) {
		m_x_sparse = Utils.createSpVctAmazon(spVct);
		for(_SparseFeature fv:m_x_sparse)
			m_totalLength += fv.getValue();
	}
	...
	// added by Md. Mustafizur Rahman for newEgg
	public void setSentencesneg(ArrayList<_SparseFeature[]> stnList) {
		m_sentences_neg = stnList.toArray(new _SparseFeature [stnList.size()][]);
	}
	...
	// added by Md. Mustafizur Rahman for HTMM Topic Modelling 
	public _SparseFeature[][] getAllSentences() {
			return this.m_sentences;
	}
	
	
	// added by Md. Mustafizur Rahman for HTMM Topic Modelling 
	public _SparseFeature[][] getAllSentencesneg() {
				return this.m_sentences_neg;
	}
	
	// added by Md. Mustafizur Rahman for HTMM Topic Modelling 
	public _SparseFeature[] getSentencesneg(int index) {
		return this.m_sentences_neg[index];
	}
	...
	public void setTopics(int k, double beta) {
		if (m_topics==null || m_topics.length!=k) {
			...
			m_topic_attribute_sstat = new double[k][2];// 2 attributes
		}
		Utils.randomize(m_topics, beta);
		Arrays.fill(m_sstat, beta);
		for(int i=0;i<k;i++)
			Arrays.fill(m_topic_attribute_sstat[i], 0.0);
	}	
